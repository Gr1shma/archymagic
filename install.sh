...
pacstrap /mnt base base-devel linux linux-firmware linux-headers intel-ucode networkmanager bluez bluez-utils git
...
pacman -S --noconfirm ntfs-3g zsh \
    ...
    noto-fonts noto-fonts-emoji noto-fonts-cjk ttf-jetbrains-mono ttf-nerd-fonts-symbols \
    mpv zathura zathura-pdf-mupdf ffmpeg imagemagick \
    fzf man-db python-pywal swayidle \
    zip unzip unrar p7zip brightnessctl udisks2 \
    dosfstools pipewire pipewire-pulse wireplumber rsync dash \
    ripgrep libnotify dunst jq aria2 \
    pamixer pulsemixer \
    zsh-syntax-highlighting xdg-user-dirs libconfig \
    bluez bluez-utils \
    uwsm xdg-desktop-portal-hyprland wl-clipboard \
    cliphist fastfetch yazi fd chafa \
    mpd mpc ncmpcpp btop htop nsxiv \
    hyprshot poppler
...
paru -S --noconfirm neovim github-cli tmux nodejs npm pnpm yarn \
    qbittorrent syncthing \
    nvidia-open nvidia-utils nvidia-settings auto-cpufreq \
    go luarocks lua51 \
    stylua prettier shfmt \
    lazygit tree zig \
    fsel-git otter-launcher cclip \
    zen-browser-bin wl-screenrec \
    rmpc mpd-sima \
    zathura-cb-git \
    tokei localsend telegram-desktop
...
cd $HOME
mkdir -p dl dox music pix vid code projects personal
chsh -s $(which zsh)
